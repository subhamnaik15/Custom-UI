...
public class Height extends View {
    ...
    public Height(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            indicatorWidth = a.getFloat(R.styleable.Height_indicator_width, indicatorWidth);
            ...
            minHeight = a.getInteger(R.styleable.Height_minHeight, minHeight);
            ...
            mMaxImage = ((BitmapDrawable) a.getDrawable(R.styleable.Height_maxImage)).getBitmap();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mMaxImage != null)
            drawIcon(mMaxImage, canvas, mProgressSweep);
    }

    private void drawIcon(Bitmap bitmap, Canvas canvas, float mProgressSweep) {
        int half = canvas.getWidth() / 2;
        if ((scrHeight - mProgressSweep) < minHeight)
            mProgressSweep = minHeight;
        else
            mProgressSweep = (scrHeight - mProgressSweep) >= (scrHeight - 5) ? scrHeight - 5 : scrHeight - mProgressSweep;

        bitmap = getResizedBitmap(bitmap, (int) mProgressSweep, half);
        canvas.drawBitmap(bitmap, null, new RectF((half), canvas.getHeight() - bitmap.getHeight(), (canvas.getWidth() / 3) + bitmap.getWidth(), canvas.getHeight()), null);
    }
    ...
    public interface OnValuesChangeListener {
         ...
         */
        void onPointsChanged(Height boxedPoints, int points);

        void onStartTrackingTouch(Height boxedPoints);

        void onStopTrackingTouch(Height boxedPoints);
    }
